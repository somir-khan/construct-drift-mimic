...
a local Ollama LLM (default: gemma4:e4b) to classify the drift type. Results
...
Usage:
    python scripts/judge_llm.py
    python scripts/judge_llm.py --model gemma4:e4b
    python scripts/judge_llm.py \\
        --samples-csv data/judge_samples.csv \\
        --results-csv data/judge_results.csv
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--surface-stats-csv",
        default="data/surface_features.csv",
        help=(
...
# System prompt
# ---------------------------------------------------------------------------
def build_surface_context(anchor_year_group: str, surface_stats: dict) -> str:
    """
    ...
    Args:
        ...
        surface_stats: dict with keys matching F1-F5 feature names.
                       If empty or None, returns a fallback string.
    ...
    phi_density     = surface_stats.get("phi_density", "N/A")
    ...
    return (
        ...
        f"(negative control — expect stable)\n"
        f"  PHI placeholder density : {fmt(phi_density)}"
    )
...
def build_system_prompt(exemplar_block: str, surface_context: str = "") -> str:
    return (
        ...
        "not a drift signal.\n"
        + exemplar_block +
        "\n\nOutput ONLY a raw JSON object with exactly three keys:\n"
        "- \"category\": one of [\"Structural Drift\", \"Lexical Drift\", "
        "\"Unresolved\"]\n"
        "- \"confidence\": one of [\"High\", \"Medium\", \"Low\"]\n"
        "- \"reasoning\": 1-2 sentences explaining your classification, citing "
...
def _strip_thinking(text: str) -> str:
    """Remove Gemma 4 thinking blocks before JSON parsing."""
...
def _parse_response(raw_text):
    """Attempt to parse a JSON classification response from the LLM."""
    cleaned = _strip_thinking(raw_text)
    for candidate in (cleaned.strip(), _strip_fences(cleaned)):
        try:
...
# Classification
# ---------------------------------------------------------------------------
def _single_classify_attempt(model, messages, max_retries, temperature):
    """Run one classification attempt with retry logic.

    Returns:
        tuple of (result_dict, error_str) for a single attempt sequence.
        result_dict has keys 'category', 'confidence', 'reasoning'
    ...
    for attempt in range(1, max_retries + 1):
        ...
        log.warning(
            "JSON parse failed on attempt %d/%d. Raw: %.120s ...",
            attempt, max_retries, raw_response,
        )

    return {"category": "", "confidence": "", "reasoning": ""}, raw_response


def _deterministic_classify(model, messages, max_retries):
    """
...
def classify_note(model, messages, max_retries, n_runs=5, temperature=0.7):
    ...
    for _ in range(n_runs):
        result, error = _single_classify_attempt(model, messages, max_retries, temperature)
        if result["category"]:
    ...
    consistency     = category_counts[modal_category] / len(all_results)
    ...
    det_result, det_error = _deterministic_classify(model, messages, max_retries)
...
def main():
    ...
    if len(df_exemplars) == 0:
        ...
        for i, (_, erow) in enumerate(df_exemplars.iterrows(), start=1):
            text_snippet = str(erow.get("text", ""))[:1500]
            parts.append(
    ...
    if os.path.exists(args.surface_stats_csv):
        ...
        for _, srow in df_surf.iterrows():
            window = str(srow.get("anchor_year_group", "")).strip()
            if window:
    ...
    for window in unique_windows:
        ...
        ctx    = build_surface_context(str(window), stats)
    ...
    try:
        ollama.chat(
            ...
            options={"num_ctx": 32768},  # ← ADD THIS
        )
    ...
    n_success = int((df_results["error"] == "").sum())
